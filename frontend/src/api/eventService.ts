...
const apiKey = localStorage.getItem('apiKey'); // Замени на свой API key

const API_BASE_URL = 'http://localhost:3000/private'; // Замените на URL вашего бэкенда (например, 'http://localhost:3000')
...
export const createEvent = async (eventData: Omit<Event, 'id'>): Promise<Event> => { // Бэкенд сам генерирует ID
  ...
  export const updateEvent = async (eventId: number, eventData: Omit<Event, 'id' | 'createdBy'>): Promise<Event> => { // Нельзя менять ID и createdBy
    const token = getAuthToken();  // Получаем токен
